...
COMMON_KWARGS = dict(
    ...
    close_mosaic = 20,
    patience     = 40,       # l 模型收敛慢，patience 适当放宽
    save_period  = 10,
    plots        = True,
)
# ─────────────────────────────────────────────────────────────────────────────
...
def train():
    ...
    s2_last = os.path.join(PROJECT,'runs/detect/' + NAME + '_s2', 'weights', 'last.pt')
    s1_best = os.path.join(PROJECT, 'runs/detect/' + NAME + '_s1', 'weights', 'best.pt')
    s1_last = os.path.join(PROJECT, 'runs/detect/' + NAME + '_s1', 'weights', 'last.pt')
