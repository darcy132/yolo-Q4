...
def train():
    checkpoint = 'runs/detect/runs/resume/road_damage_v2/weights/best.pt' # 请根据实际路径调整
    ...
    model = YOLO(checkpoint)  # 可选: yolov8n/s/m/l/x，精度要求高用 l 或 x
    results = model.train(
        ...
        mosaic=0.8,
        mixup=0.15,
        copy_paste=0.2,     # 对小样本类别（hbgdf:47个）有效
        degrees=10.0,
        flipud=0.1,
        fliplr=0.5,
        scale=0.5,
