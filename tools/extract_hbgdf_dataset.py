...
2. 过滤：只保留标注中 含有 class 1 的图片
3. 清洗：把保留图片的标注文件里非 class 1 的行去掉，class 1 重映射为 0
4. 按 8:2 划分 train/val
5. 写出到 dataset_hbgdf/ 目录
"""
...
OUTPUT_DATASET = "./dataset_hbgdf"       # 输出目录
TARGET_CLASS   = 1                    # hbgdf 红白杆倒伏
...
def main():
    ...
    # Step 2: 过滤含 class 1 的图片
    hbgdf_pairs = [(img, lbl) for img, lbl in all_pairs
                if has_target_class(lbl, TARGET_CLASS)]
    print(f"[INFO] 含 hbgdf 类别的图片数: {len(hbgdf_pairs)}")

    if not hbgdf_pairs:
        print("[ERROR] 没有找到含 hbgdf 类别的图片，请检查数据集路径和类别 ID")
        return

    # Step 3: 按 8:2 划分
    random.shuffle(hbgdf_pairs)
    n_train = int(len(hbgdf_pairs) * TRAIN_RATIO)
    train_pairs = hbgdf_pairs[:n_train]
    val_pairs   = hbgdf_pairs[n_train:]
